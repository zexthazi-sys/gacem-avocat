(function () {
  ...
  /* ── Timings (ms) ──
     ...
     Phase 2 · 350ms      : logo apparaît (opacity fade-in)
     Phase 3 · 850ms      : cercles s'agrandissent
     Phase 4 · 1600ms     : fondu de sortie + site révélé
     Phase 5 · 2000ms     : nettoyage DOM
     Mobile : timings × 4/6 (plus rapide)
  ── */
  const mobile = window.innerWidth < 768;
  const s      = mobile ? 4 / 6 : 1;

  const T_LOGO_SHOW     = Math.round(350   * s);
  const T_CIRCLES_START = Math.round(850   * s);
  const CIRCLE_DUR      = Math.round(850   * s) + 'ms';
  const T_FADE_START    = Math.round(1600  * s);
  const T_REMOVE        = Math.round(2000  * s);
